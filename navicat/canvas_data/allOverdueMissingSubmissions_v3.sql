...
SELECT 
  ...
  subs.assignment_group_total_weight AS assignment_group_total_weight,
	(select count(*) from vw_submission_state s where s.enrollment_id = subs.enrollment_id AND s.calculated_due_state = 'due') AS due_this_unit,
	(select count(*) from vw_submission_state s where s.enrollment_id = subs.enrollment_id AND s.calculated_due_state = 'due' AND s.simple_submitted_state = '2_submitted') AS submitted_this_unit,
	(select count(*) from vw_submission_state s where s.enrollment_id = subs.enrollment_id AND s.calculated_due_state = 'due' AND s.simple_submitted_state = '1_unsubmitted') AS overdue_this_unit,
	(select count(*) from vw_submission_state s where s.enrollment_id = subs.enrollment_id AND s.calculated_late_state = 'late' AND s.simple_submitted_state = '2_submitted') AS submitted_late_this_unit,
	(select count(*) from vw_submission_state s where s.user_id = subs.user_id AND s.enrollment_created_at >= @enrollmentsFromDate AND s.calculated_due_state = 'due') AS due_current_units,
	(select count(*) from vw_submission_state s where s.user_id = subs.user_id AND s.enrollment_created_at >= @enrollmentsFromDate AND s.calculated_due_state = 'due' AND s.simple_submitted_state = '2_submitted') AS submitted_current_units,
	(select count(*) from vw_submission_state s where s.user_id = subs.user_id AND s.enrollment_created_at >= @enrollmentsFromDate AND s.calculated_due_state = 'due' AND s.simple_submitted_state = '1_unsubmitted') AS overdue_current_units,
	(select count(*) from vw_submission_state s where s.user_id = subs.user_id AND s.enrollment_created_at >= @enrollmentsFromDate AND s.calculated_late_state = 'late' AND s.simple_submitted_state = '2_submitted') AS submitted_late_current_units,
	...
	(select count(*) from vw_submission_state s where s.user_id = subs.user_id AND s.enrollment_created_at < @enrollmentsFromDate AND s.calculated_late_state = 'late' AND s.simple_submitted_state = '2_submitted') AS submitted_late_prior_units,
	(select count(*) from vw_submission_state s where s.user_id = subs.user_id AND s.enrollment_created_at < @enrollmentsFromDate AND s.calculated_late_state = 'late' AND s.simple_submitted_state = '1_unsubmitted') AS overdue_prior_units,
	subs.speedgrader_url,
	CONCAT("https://collarts.instructure.com/courses/",c.canvas_id,"/assignment/",a.canvas_id) as assignment_url,
...
WHERE subs.`calculated_due_at` BETWEEN '2021-09-01' AND '2021-11-13'
AND subs.`enrollment_created_at` >= @enrollmentsFromDate
order by course_account,course_code,course_name,calculated_due_at,assignment_title,student_name;

select * from vw_submission_state_aest;