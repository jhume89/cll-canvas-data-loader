...
SET @due_from = (SELECT (Date(convert_tz(max(term.date_start), 'UTC', 'Australia/Melbourne'))) from enrollment_term_dim term where term.name = "T3 2022");
...
SELECT 
  ...
  subs.assignment_group_total_weight AS assignment_group_total_weight,
	(select count(s.enrollment_id) from vw_submission_states s where s.enrollment_id = subs.enrollment_id AND s.calculated_due_state = 'due') AS due_this_unit,
	(select count(s.enrollment_id) from vw_submission_states s where s.enrollment_id = subs.enrollment_id AND s.calculated_due_state = 'due' AND s.simple_submitted_state = '2_submitted') AS submitted_this_unit,
	(select count(s.enrollment_id) from vw_submission_states s where s.enrollment_id = subs.enrollment_id AND s.calculated_late_state = 'overdue') AS overdue_this_unit,
	(select count(s.enrollment_id) from vw_submission_states s where s.enrollment_id = subs.enrollment_id AND s.calculated_late_state = 'submitted_late') AS submitted_late_this_unit,
	(select count(s.enrollment_id) from vw_submission_states s where s.user_id = subs.user_id AND s.enrollment_created_at >= @enrollmentsFromDate AND s.calculated_due_state = 'due') AS due_current_units,
	(select count(s.enrollment_id) from vw_submission_states s where s.user_id = subs.user_id AND s.enrollment_created_at >= @enrollmentsFromDate AND s.calculated_due_state = 'due' AND s.simple_submitted_state = '2_submitted') AS submitted_current_units,
	(select count(s.enrollment_id) from vw_submission_states s where s.user_id = subs.user_id AND s.enrollment_created_at >= @enrollmentsFromDate AND s.calculated_late_state = 'overdue') AS overdue_current_units,
	(select count(s.enrollment_id) from vw_submission_states s where s.user_id = subs.user_id AND s.enrollment_created_at >= @enrollmentsFromDate AND s.calculated_late_state = 'submitted_late') AS submitted_late_current_units,
	(select count(DISTINCT (s.enrollment_id)) from vw_submission_states s where s.user_id = subs.user_id AND s.enrollment_created_at >= @enrollmentsFromDate) AS count_current_enrollments,
	(select count(DISTINCT (s.enrollment_id)) from vw_submission_states s where s.user_id = subs.user_id) AS count_alltime_enrollments,
	(select count(s.enrollment_id) from vw_submission_states s where s.user_id = subs.user_id AND s.enrollment_created_at < @enrollmentsFromDate AND s.calculated_late_state = 'submitted_late') AS submitted_late_prior_units,
	(select count(s.enrollment_id) from vw_submission_states s where s.user_id = subs.user_id AND s.enrollment_created_at < @enrollmentsFromDate AND s.calculated_late_state = 'overdue') AS overdue_prior_units,
