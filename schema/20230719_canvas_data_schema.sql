...
CREATE TABLE `assignment_dim` (
  ...
  `workflow_state` enum('unpublished','published','deleted','fail_to_import','failed_to_duplicate','duplicating') DEFAULT NULL,
...
CREATE TABLE `cll_mv_major_degrees` (
  ...
  `all_account_names` mediumtext DEFAULT NULL,
  `enrollment_split` mediumtext DEFAULT NULL,
  `latest_enrollment_terms` mediumtext DEFAULT NULL,
...
CREATE TABLE `cll_mv_submission_states` (
  ...
  `enrollment_created_at` binary(0) DEFAULT NULL,
  ...
  `user_id` bigint(20) unsigned NOT NULL,
  `enrollment_id` binary(0) DEFAULT NULL,
  `course_id` bigint(20) unsigned NOT NULL,
...
CREATE TABLE `communication_channel_dim` (
  ...
  `type` enum('email','facebook','push','sms','twitter','slack') DEFAULT NULL,
  `position` int(10) unsigned DEFAULT NULL,
...
CREATE TABLE `submission_dim` (
  ...
  `assignment_id` bigint(20) unsigned DEFAULT NULL,
  `quiz_submission_id` bigint(20) unsigned DEFAULT NULL,
  `submission_type` enum('discussion_topic','external_tool','media_recording','online_file_upload','online_quiz','online_text_entry','online_upload','online_url','basic_lti_launch','student_annotation') DEFAULT NULL,
  `workflow_state` enum('graded','pending_review','submitted','unsubmitted','deleted') DEFAULT NULL,
  `attempt` int(10) unsigned DEFAULT NULL,
  `grade_matches_current_submission` enum('false','true') DEFAULT NULL,
  `created_at` datetime DEFAULT NULL,
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `ls_monitored_accounts_he` AS select `acc`.`id` AS `id`,`acc`.`id` AS `account_id`,`acc`.`name` AS `NAME` from `account_dim` `acc` where `acc`.`name` not in ('Production FOLD','Sandbox','Sample theme','VETDSS','VET','Templates','JH Account','Student Resources','WIL','Training','Test','Manually-Created Courses','Ruth Fleishman sandbox','Short Courses','Exemplar','Collarts Resources','Unit Refresh','Dev','Onboarding','ED TECH','PROJECTS','ORIENTATION');

-- ----------------------------
-- View structure for vw_assignment_due_dates
-- ----------------------------
DROP VIEW IF EXISTS `vw_assignment_due_dates`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_assignment_due_dates` AS select `u`.`id` AS `user_id`,`staleA`.`course_id` AS `course_id`,NULL AS `enrollment_id`,`staleA`.`id` AS `assignment_id`,`liveA`.`canvas_id` AS `assignment_canvas_id`,`c`.`enrollment_term_id` AS `enrollment_term_id`,NULL AS `enrollment_created_at`,`liveC`.`start_at` AS `course_start`,`liveC`.`end_at` AS `course_end`,`t`.`date_start` AS `term_start`,`t`.`date_end` AS `term_end`,`ovu`.`ov_id` AS `assignment_override_id`,`liveA`.`unlock_at` AS `original_unlock_at`,`liveA`.`due_at` AS `original_due_at`,`liveA`.`lock_at` AS `original_lock_at`,`ov`.`unlock_at` AS `override_unlock_at`,`ov`.`due_at` AS `override_due_at`,`ov`.`lock_at` AS `override_lock_at`,`ov`.`unlock_at_overridden` AS `overridden_unlock_flag`,`ov`.`due_at_overridden` AS `overridden_due_flag`,`ov`.`lock_at_overridden` AS `overridden_lock_flag`,if(`liveC`.`restrict_enrollments_to_course_dates` = 'true' and `liveC`.`start_at` is not null,`liveC`.`start_at`,`t`.`date_start`) AS `calculated_course_start`,if(`ov`.`unlock_at` is not null,`ov`.`unlock_at`,`liveA`.`unlock_at`) AS `calculated_unlock_at`,if(`ov`.`due_at` is not null,`ov`.`due_at`,`liveA`.`due_at`) AS `calculated_due_at`,if(`ov`.`lock_at` is not null,`ov`.`lock_at`,`liveA`.`lock_at`) AS `calculated_lock_at`,if(`liveC`.`restrict_enrollments_to_course_dates` = 'true' and `liveC`.`end_at` is not null,`liveC`.`end_at`,`t`.`date_end`) AS `calculated_course_end`,if(utc_timestamp() >= if(`ov`.`due_at` is not null,`ov`.`due_at`,`liveA`.`due_at`),'due','not_yet_due') AS `calculated_due_state` from (((((((((`canvas_data`.`course_dim` `c` join `canvas_data`.`cll_courses` `liveC` on(`liveC`.`canvas_id` = `c`.`canvas_id`)) join `canvas_data`.`cll_assignments` `liveA` on(`liveA`.`course_canvas_id` = `c`.`canvas_id` and `liveA`.`workflow_state` <> 'deleted' and `c`.`workflow_state` <> 'deleted')) join `canvas_data`.`enrollment_term_dim` `t` on(`t`.`id` = `c`.`enrollment_term_id`)) join (select distinct `e2`.`user_id` AS `user_id`,`e2`.`course_id` AS `course_id` from `canvas_data`.`enrollment_dim` `e2` where `e2`.`type` = 'StudentEnrollment' and `e2`.`workflow_state` not in ('deleted','inactive','rejected') order by `e2`.`course_section_id`) `e` on(`e`.`course_id` = `c`.`id`)) join `canvas_data`.`user_dim` `u` on(`u`.`id` = `e`.`user_id`)) join `canvas_data`.`cll_submissions` `liveS` on(`liveS`.`assignment_canvas_id` = `liveA`.`canvas_id` and `liveS`.`user_canvas_id` = `u`.`canvas_id`)) left join `canvas_data`.`assignment_dim` `staleA` on(`staleA`.`canvas_id` = `liveS`.`assignment_canvas_id`)) left join `canvas_data`.`vw_assignment_override_per_student` `ovu` on(`ovu`.`user_id` = `e`.`user_id` and `ovu`.`assignment_id` = `staleA`.`id` and exists(select 1 from `canvas_data`.`assignment_override_dim` `ao` where `ao`.`id` = `ovu`.`ov_id` and `ao`.`workflow_state` <> 'deleted' limit 1))) left join `canvas_data`.`assignment_override_dim` `ov` on(`ov`.`id` = `ovu`.`ov_id`));

-- ----------------------------
-- View structure for vw_assignment_due_dates_bak
-- ----------------------------
DROP VIEW IF EXISTS `vw_assignment_due_dates_bak`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_assignment_due_dates_bak` AS select `e`.`user_id` AS `user_id`,`staleA`.`course_id` AS `course_id`,`e`.`id` AS `enrollment_id`,`staleA`.`id` AS `assignment_id`,`liveA`.`canvas_id` AS `assignment_canvas_id`,`c`.`enrollment_term_id` AS `enrollment_term_id`,`e`.`created_at` AS `enrollment_created_at`,`liveC`.`start_at` AS `course_start`,`liveC`.`end_at` AS `course_end`,`t`.`date_start` AS `term_start`,`t`.`date_end` AS `term_end`,`ovu`.`assignment_override_id` AS `assignment_override_id`,`liveA`.`unlock_at` AS `original_unlock_at`,`liveA`.`due_at` AS `original_due_at`,`liveA`.`lock_at` AS `original_lock_at`,`ov`.`unlock_at` AS `override_unlock_at`,`ov`.`due_at` AS `override_due_at`,`ov`.`lock_at` AS `override_lock_at`,`ov`.`unlock_at_overridden` AS `overridden_unlock_flag`,`ov`.`due_at_overridden` AS `overridden_due_flag`,`ov`.`lock_at_overridden` AS `overridden_lock_flag`,if(`liveC`.`restrict_enrollments_to_course_dates` = 'true' and `liveC`.`start_at` is not null,`liveC`.`start_at`,`t`.`date_start`) AS `calculated_course_start`,if(`ov`.`unlock_at` is not null,`ov`.`unlock_at`,`liveA`.`unlock_at`) AS `calculated_unlock_at`,if(`ov`.`due_at` is not null,`ov`.`due_at`,`liveA`.`due_at`) AS `calculated_due_at`,if(`ov`.`lock_at` is not null,`ov`.`lock_at`,`liveA`.`lock_at`) AS `calculated_lock_at`,if(`liveC`.`restrict_enrollments_to_course_dates` = 'true' and `liveC`.`end_at` is not null,`liveC`.`end_at`,`t`.`date_end`) AS `calculated_course_end`,if(utc_timestamp() >= if(`ov`.`due_at` is not null,`ov`.`due_at`,`liveA`.`due_at`),'due','not_yet_due') AS `calculated_due_state` from (((((((`course_dim` `c` join `cll_courses` `liveC` on(`liveC`.`canvas_id` = `c`.`canvas_id`)) join `cll_assignments` `liveA` on(`liveA`.`course_canvas_id` = `c`.`canvas_id` and `liveA`.`workflow_state` <> 'deleted' and `c`.`workflow_state` <> 'deleted')) join `assignment_dim` `staleA` on(`staleA`.`canvas_id` = `liveA`.`canvas_id`)) join `enrollment_term_dim` `t` on(`t`.`id` = `c`.`enrollment_term_id`)) left join `enrollment_dim` `e` on(`e`.`course_id` = `c`.`id` and `e`.`type` = 'StudentEnrollment' and `e`.`workflow_state` not in ('deleted','inactive','rejected'))) left join `assignment_override_user_dim` `ovu` on(`ovu`.`user_id` = `e`.`user_id` and `ovu`.`assignment_id` = `staleA`.`id` and exists(select 1 from `assignment_override_dim` `ao` where `ao`.`id` = `ovu`.`assignment_override_id` and `ao`.`workflow_state` <> 'deleted' limit 1))) left join `assignment_override_dim` `ov` on(`ov`.`id` = `ovu`.`assignment_override_id`));

-- ----------------------------
-- View structure for vw_assignment_due_dates_lovellfix
-- ----------------------------
DROP VIEW IF EXISTS `vw_assignment_due_dates_lovellfix`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_assignment_due_dates_lovellfix` AS select `u`.`id` AS `user_id`,`staleA`.`course_id` AS `course_id`,NULL AS `enrollment_id`,`staleA`.`id` AS `assignment_id`,`liveA`.`canvas_id` AS `assignment_canvas_id`,`c`.`enrollment_term_id` AS `enrollment_term_id`,NULL AS `enrollment_created_at`,`liveC`.`start_at` AS `course_start`,`liveC`.`end_at` AS `course_end`,`t`.`date_start` AS `term_start`,`t`.`date_end` AS `term_end`,`ovu`.`ov_id` AS `assignment_override_id`,`liveA`.`unlock_at` AS `original_unlock_at`,`liveA`.`due_at` AS `original_due_at`,`liveA`.`lock_at` AS `original_lock_at`,`ov`.`unlock_at` AS `override_unlock_at`,`ov`.`due_at` AS `override_due_at`,`ov`.`lock_at` AS `override_lock_at`,`ov`.`unlock_at_overridden` AS `overridden_unlock_flag`,`ov`.`due_at_overridden` AS `overridden_due_flag`,`ov`.`lock_at_overridden` AS `overridden_lock_flag`,if(`liveC`.`restrict_enrollments_to_course_dates` = 'true' and `liveC`.`start_at` is not null,`liveC`.`start_at`,`t`.`date_start`) AS `calculated_course_start`,if(`ov`.`unlock_at` is not null,`ov`.`unlock_at`,`liveA`.`unlock_at`) AS `calculated_unlock_at`,if(`ov`.`due_at` is not null,`ov`.`due_at`,`liveA`.`due_at`) AS `calculated_due_at`,if(`ov`.`lock_at` is not null,`ov`.`lock_at`,`liveA`.`lock_at`) AS `calculated_lock_at`,if(`liveC`.`restrict_enrollments_to_course_dates` = 'true' and `liveC`.`end_at` is not null,`liveC`.`end_at`,`t`.`date_end`) AS `calculated_course_end`,if(utc_timestamp() >= if(`ov`.`due_at` is not null,`ov`.`due_at`,`liveA`.`due_at`),'due','not_yet_due') AS `calculated_due_state` from (((((((((`canvas_data`.`course_dim` `c` join `canvas_data`.`cll_courses` `liveC` on(`liveC`.`canvas_id` = `c`.`canvas_id`)) join `canvas_data`.`cll_assignments` `liveA` on(`liveA`.`course_canvas_id` = `c`.`canvas_id` and `liveA`.`workflow_state` <> 'deleted' and `c`.`workflow_state` <> 'deleted')) join `canvas_data`.`enrollment_term_dim` `t` on(`t`.`id` = `c`.`enrollment_term_id`)) join (select distinct `e2`.`user_id` AS `user_id`,`e2`.`course_id` AS `course_id` from `canvas_data`.`enrollment_dim` `e2` where `e2`.`type` = 'StudentEnrollment' and `e2`.`workflow_state` not in ('deleted','inactive','rejected') order by `e2`.`course_section_id`) `e` on(`e`.`course_id` = `c`.`id`)) join `canvas_data`.`user_dim` `u` on(`u`.`id` = `e`.`user_id`)) join `canvas_data`.`cll_submissions` `liveS` on(`liveS`.`assignment_canvas_id` = `liveA`.`canvas_id` and `liveS`.`user_canvas_id` = `u`.`canvas_id`)) left join `canvas_data`.`assignment_dim` `staleA` on(`staleA`.`canvas_id` = `liveS`.`assignment_canvas_id`)) left join `canvas_data`.`vw_assignment_override_per_student` `ovu` on(`ovu`.`user_id` = `e`.`user_id` and `ovu`.`assignment_id` = `staleA`.`id` and exists(select 1 from `canvas_data`.`assignment_override_dim` `ao` where `ao`.`id` = `ovu`.`ov_id` and `ao`.`workflow_state` <> 'deleted' limit 1))) left join `canvas_data`.`assignment_override_dim` `ov` on(`ov`.`id` = `ovu`.`ov_id`));

-- ----------------------------
-- View structure for vw_assignment_due_dates_lovellfix_dupenrl_bak
-- ----------------------------
DROP VIEW IF EXISTS `vw_assignment_due_dates_lovellfix_dupenrl_bak`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_assignment_due_dates_lovellfix_dupenrl_bak` AS select `e`.`user_id` AS `user_id`,`staleA`.`course_id` AS `course_id`,`e`.`id` AS `enrollment_id`,`staleA`.`id` AS `assignment_id`,`liveA`.`canvas_id` AS `assignment_canvas_id`,`c`.`enrollment_term_id` AS `enrollment_term_id`,`e`.`created_at` AS `enrollment_created_at`,`liveC`.`start_at` AS `course_start`,`liveC`.`end_at` AS `course_end`,`t`.`date_start` AS `term_start`,`t`.`date_end` AS `term_end`,`ovu`.`ov_id` AS `assignment_override_id`,`liveA`.`unlock_at` AS `original_unlock_at`,`liveA`.`due_at` AS `original_due_at`,`liveA`.`lock_at` AS `original_lock_at`,`ov`.`unlock_at` AS `override_unlock_at`,`ov`.`due_at` AS `override_due_at`,`ov`.`lock_at` AS `override_lock_at`,`ov`.`unlock_at_overridden` AS `overridden_unlock_flag`,`ov`.`due_at_overridden` AS `overridden_due_flag`,`ov`.`lock_at_overridden` AS `overridden_lock_flag`,if(`liveC`.`restrict_enrollments_to_course_dates` = 'true' and `liveC`.`start_at` is not null,`liveC`.`start_at`,`t`.`date_start`) AS `calculated_course_start`,if(`ov`.`unlock_at` is not null,`ov`.`unlock_at`,`liveA`.`unlock_at`) AS `calculated_unlock_at`,if(`ov`.`due_at` is not null,`ov`.`due_at`,`liveA`.`due_at`) AS `calculated_due_at`,if(`ov`.`lock_at` is not null,`ov`.`lock_at`,`liveA`.`lock_at`) AS `calculated_lock_at`,if(`liveC`.`restrict_enrollments_to_course_dates` = 'true' and `liveC`.`end_at` is not null,`liveC`.`end_at`,`t`.`date_end`) AS `calculated_course_end`,if(utc_timestamp() >= if(`ov`.`due_at` is not null,`ov`.`due_at`,`liveA`.`due_at`),'due','not_yet_due') AS `calculated_due_state` from (((((((((`canvas_data`.`course_dim` `c` join `canvas_data`.`cll_courses` `liveC` on(`liveC`.`canvas_id` = `c`.`canvas_id`)) join `canvas_data`.`cll_assignments` `liveA` on(`liveA`.`course_canvas_id` = `c`.`canvas_id` and `liveA`.`workflow_state` <> 'deleted' and `c`.`workflow_state` <> 'deleted')) join `canvas_data`.`enrollment_term_dim` `t` on(`t`.`id` = `c`.`enrollment_term_id`)) join `canvas_data`.`enrollment_dim` `e` on(`e`.`course_id` = `c`.`id` and `e`.`type` = 'StudentEnrollment' and `e`.`workflow_state` not in ('deleted','inactive','rejected'))) join `canvas_data`.`user_dim` `u` on(`u`.`id` = `e`.`user_id`)) join `canvas_data`.`cll_submissions` `liveS` on(`liveS`.`assignment_canvas_id` = `liveA`.`canvas_id` and `liveS`.`user_canvas_id` = `u`.`canvas_id`)) left join `canvas_data`.`assignment_dim` `staleA` on(`staleA`.`canvas_id` = `liveS`.`assignment_canvas_id`)) left join `canvas_data`.`vw_assignment_override_per_student` `ovu` on(`ovu`.`user_id` = `e`.`user_id` and `ovu`.`assignment_id` = `staleA`.`id` and exists(select 1 from `canvas_data`.`assignment_override_dim` `ao` where `ao`.`id` = `ovu`.`ov_id` and `ao`.`workflow_state` <> 'deleted' limit 1))) left join `canvas_data`.`assignment_override_dim` `ov` on(`ov`.`id` = `ovu`.`ov_id`));

-- ----------------------------
-- View structure for vw_assignment_override_maxdue_student
-- ----------------------------
DROP VIEW IF EXISTS `vw_assignment_override_maxdue_student`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_assignment_override_maxdue_student` AS select `maxdue`.`user_id` AS `user_id`,`maxdue`.`assignment_id` AS `assignment_id`,(select `m2`.`ovu_id` from `canvas_data`.`vw_assignment_override_multi_student` `m2` where `m2`.`user_id` = `maxdue`.`user_id` and `m2`.`assignment_id` = `maxdue`.`assignment_id` and `m2`.`due_at` = `maxdue`.`due_at` order by `m2`.`ovu_updated_at` desc,`m2`.`ovu_id` desc limit 1) AS `ovu_id` from (select `m3`.`user_id` AS `user_id`,`m3`.`assignment_id` AS `assignment_id`,max(`m3`.`due_at`) AS `due_at` from `canvas_data`.`vw_assignment_override_multi_student` `m3` group by `m3`.`user_id`,`m3`.`assignment_id`) `maxdue`;

-- ----------------------------
-- View structure for vw_assignment_override_multi_student
-- ----------------------------
DROP VIEW IF EXISTS `vw_assignment_override_multi_student`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_assignment_override_multi_student` AS select `ovu`.`id` AS `ovu_id`,`ovu`.`user_id` AS `user_id`,`ov`.`assignment_id` AS `assignment_id`,`ovu`.`created_at` AS `ovu_created_at`,`ovu`.`updated_at` AS `ovu_updated_at`,`ov`.`id` AS `ov_id`,`ov`.`due_at` AS `due_at`,`ov`.`created_at` AS `ov_created_at`,`ov`.`updated_at` AS `ov_updated_at` from (`assignment_override_user_dim` `ovu` join `assignment_override_dim` `ov` on(`ovu`.`assignment_override_id` = `ov`.`id`));

-- ----------------------------
-- View structure for vw_assignment_override_per_student
-- ----------------------------
DROP VIEW IF EXISTS `vw_assignment_override_per_student`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_assignment_override_per_student` AS select `ovu`.`id` AS `ovu_id`,`ovu`.`user_id` AS `user_id`,`ov`.`assignment_id` AS `assignment_id`,`ovu`.`created_at` AS `ovu_created_at`,`ovu`.`updated_at` AS `ovu_updated_at`,`ov`.`id` AS `ov_id`,`ov`.`due_at` AS `due_at`,`ov`.`created_at` AS `ov_created_at`,`ov`.`updated_at` AS `ov_updated_at` from ((`canvas_data`.`assignment_override_user_dim` `ovu` join `canvas_data`.`assignment_override_dim` `ov` on(`ovu`.`assignment_override_id` = `ov`.`id`)) join `canvas_data`.`vw_assignment_override_maxdue_student` `maxdue` on(`maxdue`.`ovu_id` = `ovu`.`id`));
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_degree_prevalence` AS select `p`.`account_id` AS `account_id`,`p`.`account_name` AS `account_name`,`p`.`degree_name` AS `degree_name`,`u`.`id` AS `user_id`,`u`.`canvas_id` AS `user_canvas_id`,`u`.`name` AS `user_name`,`p`.`enrollment_count` AS `enrollment_count`,`p`.`core_enrollment_count` AS `core_enrollment_count`,`p`.`ia_enrollment_count` AS `ia_enrollment_count`,`p`.`other_enrollment_count` AS `other_enrollment_count`,`p`.`latest_enrollment_term` AS `latest_enrollment_term`,`p`.`latest_enrollment_at` AS `latest_enrollment_at`,`p`.`course_codes` AS `course_codes`,`p`.`enrollment_dates` AS `enrollment_dates` from (`canvas_data`.`user_dim` `u` left join (select `acc`.`id` AS `account_id`,`acc`.`name` AS `account_name`,`deg_acc`.`name` AS `degree_name`,`u`.`id` AS `user_id`,count(`e`.`id`) AS `enrollment_count`,if(`deg_acc`.`name` not in ('Sandbox','Dev','Student Resources','Industry Awareness','Training','Onboarding','CG Spectrum','WIL'),count(`e`.`id`),0) AS `core_enrollment_count`,if(`deg_acc`.`name` = 'Industry Awareness',count(`e`.`id`),0) AS `ia_enrollment_count`,if(`deg_acc`.`name` in ('Sandbox','Dev','Student Resources','Training','Onboarding','CG Spectrum','WIL'),count(`e`.`id`),0) AS `other_enrollment_count`,max(`e`.`created_at`) AS `latest_enrollment_at`,max(`et`.`sortable_name`) AS `latest_enrollment_term`,group_concat(distinct date_format(`e`.`created_at`,'%Y%m%d') order by `e`.`created_at` DESC separator '|') AS `enrollment_dates`,group_concat(`c`.`code` order by `et`.`sortable_name` DESC separator '|') AS `course_codes` from (((((`canvas_data`.`account_dim` `acc` join (select `acc2`.`id` AS `account_id`,replace(replace(`acc2`.`name`,'FOLDS: ',''),'CG Spectrum','Animation and VFX') AS `name` from `canvas_data`.`account_dim` `acc2`) `deg_acc` on(`deg_acc`.`account_id` = `acc`.`id`)) join `canvas_data`.`course_dim` `c` on(`acc`.`id` = `c`.`account_id`)) join `canvas_data`.`enrollment_dim` `e` on(`c`.`id` = `e`.`course_id`)) join `canvas_data`.`user_dim` `u` on(`e`.`user_id` = `u`.`id` and `e`.`type` = 'StudentEnrollment' and `e`.`workflow_state` = 'active')) join `canvas_data`.`vw_term_names` `et` on(`c`.`enrollment_term_id` = `et`.`term_id`)) group by `acc`.`id`,`acc`.`name`,`deg_acc`.`name`,`u`.`id`) `p` on(`p`.`user_id` = `u`.`id`)) order by `u`.`name`,`p`.`core_enrollment_count` desc,`p`.`latest_enrollment_term` desc,`p`.`latest_enrollment_at` desc,`p`.`degree_name` desc;
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vw_major_degrees` AS select `p`.`user_id` AS `user_id`,`p`.`user_canvas_id` AS `user_canvas_id`,`p`.`user_name` AS `user_name`,if(`m`.`max_core_count` > 0,group_concat(distinct `d`.`degree_name` order by `p`.`latest_enrollment_term` DESC,`p`.`latest_enrollment_at` DESC separator ','),NULL) AS `degree_name`,group_concat(`p`.`account_name` order by `p`.`core_enrollment_count` DESC,`p`.`latest_enrollment_term` DESC,`p`.`latest_enrollment_at` DESC,`p`.`degree_name` ASC separator '|') AS `all_account_names`,group_concat(`p`.`enrollment_count` order by `p`.`core_enrollment_count` DESC,`p`.`latest_enrollment_term` DESC,`p`.`latest_enrollment_at` DESC,`p`.`degree_name` ASC separator '|') AS `enrollment_split`,group_concat(`p`.`latest_enrollment_term` order by `p`.`core_enrollment_count` DESC,`p`.`latest_enrollment_term` DESC,`p`.`latest_enrollment_at` DESC,`p`.`degree_name` ASC separator '|') AS `latest_enrollment_terms`,group_concat(date_format(`p`.`latest_enrollment_at`,'%Y%m%d') order by `p`.`core_enrollment_count` DESC,`p`.`latest_enrollment_term` DESC,`p`.`latest_enrollment_at` DESC,`p`.`degree_name` ASC separator '|') AS `latest_enrollment_dates`,sum(`p`.`core_enrollment_count`) AS `total_core_enrollment_count`,max(`m`.`ia_enrollment_count`) AS `ia_enrollment_count`,max(`m`.`other_enrollment_count`) AS `other_enrollment_count`,max(`m`.`total_enrollment_count`) AS `total_enrollment_count`,group_concat(distinct `d`.`account_id` separator '|') AS `account_id` from ((`canvas_data`.`vw_degree_prevalence` `p` join (select `g`.`user_id` AS `user_id`,max(`g`.`core_enrollment_count`) AS `max_core_count`,sum(`g`.`ia_enrollment_count`) AS `ia_enrollment_count`,sum(`g`.`other_enrollment_count`) AS `other_enrollment_count`,sum(`g`.`enrollment_count`) AS `total_enrollment_count` from `canvas_data`.`vw_degree_prevalence` `g` group by `g`.`user_id`) `m` on(`m`.`user_id` = `p`.`user_id` and `m`.`max_core_count` = `p`.`core_enrollment_count`)) left join (select row_number() over ( partition by `l`.`user_id` order by `l`.`core_enrollment_count` desc,`l`.`latest_enrollment_term` desc,`l`.`latest_enrollment_at` desc,`l`.`degree_name`) AS `degree_rank`,`l`.`user_id` AS `user_id`,`l`.`degree_name` AS `degree_name`,`l`.`account_id` AS `account_id` from `canvas_data`.`vw_degree_prevalence` `l`) `d` on(`d`.`user_id` = `m`.`user_id` and `d`.`degree_rank` = 1)) group by `p`.`user_id`,`p`.`user_name` order by group_concat(distinct `d`.`degree_name` separator ','),`p`.`user_name`;
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `vxm_submission_states_wtfix` AS select convert_tz((select min(`exports`.`cll_retrieved_at`) from `cll_submissions` `exports`),'UTC','Australia/Melbourne') AS `export_cutoff_AEST`,NULL AS `enrollment_created_at`,`acc`.`name` AS `course_account`,`d`.`degree_name` AS `student_degree`,`u`.`name` AS `student_name`,`pseudo`.`sis_user_id` AS `collarts_student_id`,`u`.`id` AS `user_id`,NULL AS `enrollment_id`,`c`.`id` AS `course_id`,`c`.`code` AS `course_code`,`c`.`name` AS `course_name`,`vt`.`sortable_name` AS `term_map`,`staleA`.`id` AS `assignment_id`,`aweight`.`assignment_group_name` AS `assignment_group_name`,`liveA`.`title` AS `assignment_title`,if(`liveA`.`submission_types` <> 'external_tool',`liveA`.`submission_types`,`tool`.`name`) AS `assignment_type`,`liveA`.`workflow_state` AS `assignment_workflow_state`,`ad`.`original_due_at` AS `original_due_at`,`ad`.`override_due_at` AS `override_due_at`,`ad`.`calculated_due_at` AS `calculated_due_at`,`ad`.`calculated_due_state` AS `calculated_due_state`,case `liveS`.`workflow_state` when 'unsubmitted' then '1_unsubmitted' when 'submitted' then '2_submitted' when 'pending_review' then '3_pending_review' when 'graded' then '4_graded' else if(`liveS`.`workflow_state` is null,'0_null_submission',`liveS`.`workflow_state`) end AS `submission_workflow_state`,if(`liveS`.`submitted_at` is null,'1_unsubmitted','2_submitted') AS `simple_submitted_state`,if(`liveS`.`submitted_at` <= `ad`.`calculated_due_at`,'submitted_ontime',if(`liveS`.`submitted_at` > `ad`.`calculated_due_at`,'submitted_late',if(`liveS`.`submitted_at` is null and `liveS`.`workflow_state` = 'graded',if(`liveS`.`score` <> 0,'graded_nosub','zeroed_nosub'),if(`liveS`.`submitted_at` is null and `ad`.`calculated_due_at` + interval 2 day <= utc_timestamp(),'overdue',if(`liveS`.`submitted_at` is null and `ad`.`calculated_due_at` + interval 0 day <= utc_timestamp(),'overdue_grace48','not_yet_due'))))) AS `calculated_late_state`,if(`liveS`.`submitted_at` is null,NULL,round(timestampdiff(MINUTE,`ad`.`calculated_due_at`,`liveS`.`submitted_at`) / (24 * 60),4)) AS `calculated_late_days`,if(`liveS`.`graded_at` > `liveS`.`submitted_at`,`liveS`.`graded_at`,`liveS`.`submitted_at`) AS `calculated_submission_updated_at`,`liveS`.`submitted_at` AS `submitted_at`,`liveS`.`graded_at` AS `graded_at`,`liveA`.`assignment_group_canvas_id` AS `assignment_group_canvas_id`,`aweight`.`assignment_group_total_weight` AS `assignment_group_total_weight`,`aweight`.`calculated_assignment_weight` AS `calculated_assignment_weight`,`liveS`.`score` / nullif(`aweight`.`assignment_group_sum_possible`,0) * (`aweight`.`assignment_group_total_weight` / 100) AS `score_weighted_contribution`,`aweight`.`assignment_group_sum_possible` AS `assignment_group_sum_possible`,`liveA`.`points_possible` AS `assignment_points_possible`,`liveS`.`score` AS `score_raw`,`liveS`.`score` / nullif(`liveA`.`points_possible`,0) AS `score_percent`,`liveS`.`grade` AS `grade_raw`,concat('https://collarts.instructure.com/courses/',`c`.`canvas_id`,'/assignments/',`liveA`.`canvas_id`) AS `assignment_url`,concat('https://collarts.instructure.com/courses/',`c`.`canvas_id`,'/gradebook/speed_grader?assignment_id=',`liveA`.`canvas_id`,'&student_id=',`u`.`canvas_id`) AS `speedgrader_url`,`teachers`.`teacher_list` AS `teacher_list` from ((((((((((((`user_dim` `u` join `course_dim` `c` on(exists(select `e2`.`user_id` from `enrollment_dim` `e2` where `e2`.`course_id` = `c`.`id` and `e2`.`user_id` = `u`.`id` and `e2`.`type` = 'StudentEnrollment' and `e2`.`workflow_state` = 'active' and `e2`.`created_at` >= '2019-08-01' limit 1))) join `account_dim` `acc` on(`acc`.`id` = `c`.`account_id`)) join `cll_mv_course_teacher_list` `teachers` on(`teachers`.`course_id` = `c`.`id`)) join `cll_mv_term_names` `vt` on(`c`.`enrollment_term_id` = `vt`.`term_id`)) join `cll_assignments` `liveA` on(`c`.`canvas_id` = `liveA`.`course_canvas_id`)) join `cll_submissions` `liveS` on(`liveA`.`canvas_id` = `liveS`.`assignment_canvas_id` and `liveS`.`user_canvas_id` = `u`.`canvas_id` and (`liveS`.`workflow_state` is null or `liveS`.`workflow_state` <> 'deleted'))) join `assignment_dim` `staleA` on(`staleA`.`canvas_id` = `liveA`.`canvas_id`)) join `cll_mv_assignment_due_dates` `ad` on(`ad`.`user_id` = `u`.`id` and `ad`.`assignment_canvas_id` = `liveA`.`canvas_id`)) join `cll_mv_assignment_weights_per_student` `aweight` on(`aweight`.`submission_canvas_id` = `liveS`.`canvas_id`)) left join `cll_mv_major_degrees` `d` on(`d`.`user_id` = `u`.`id`)) left join `external_tool_activation_dim` `tool` on(`liveA`.`external_tool_canvas_id` = `tool`.`canvas_id`)) left join `pseudonym_dim` `pseudo` on(`pseudo`.`user_id` = `u`.`id` and `pseudo`.`sis_user_id` is not null)) where `liveA`.`workflow_state` = 'published';
